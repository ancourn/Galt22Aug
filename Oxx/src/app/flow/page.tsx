...
import { AutomationBuilder } from '@/components/ui/automation-builder';
...
export default function FlowPage() {
  ...
  const loadAutomations = async () => {
    try {
      const response = await fetch('/api/flow/automations');
      if (response.ok) {
  ...
  const loadExecutions = async () => {
    try {
      const response = await fetch('/api/flow/executions/recent');
      if (response.ok) {
        const data = await response.json();
        setExecutions(data);
      }
  ...
  const handleSaveAutomation = async (automation: Partial<Automation>) => {
    try {
      const url = editingAutomation ? `/api/flow/automations/${editingAutomation.id}` : '/api/flow/automations';
  ...
  const handleTestAutomation = async (automation: Partial<Automation>) => {
    try {
      const response = await fetch(`/api/flow/automations/${automation.id}/test`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      alert('Automation test failed. Check console for details.');
    }
  };

  const handleToggleAutomation = async (automation: Automation) => {
    try {
      const response = await fetch(`/api/flow/automations/${automation.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const getTriggerLabel = (type: string) => {
    switch (type) {
      case 'formbricks_score': return 'Formbricks Score';
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center gap-4 text-sm">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Badge variant="outline">{automation.actions.length}</Badge>
                              </div>
                              <div className="text-muted-foreground">
                                Created: {formatTime(automation.createdAt)}
                              </div>
                            </div>
                            
                            <div className="text-sm text-muted-foreground">
                              <strong>Trigger:</strong> {getTriggerLabel(automation.trigger.type)}
                              {automation.trigger.condition && (
                                <span className="ml-2">
                                  ({JSON.stringify(automation.trigger.condition)})
                                </span>
                              )}
                            </div>
                            
                            <div className="text-sm text-muted-foreground">
                              <strong>Actions:</strong> {automation.actions.map(a => a.type).join(', ')}
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                    ...
                    <Card key={execution.id}>
                      ...
                      <CardContent>
                        <div className="space-y-2 text-sm">
                          ...
                          <div>
                            <strong>Actions:</strong> {execution.actionResults?.length || 0}
                          </div>
                          <div>
                            <strong>Success:</strong> {execution.actionResults?.filter(r => r.success).length || 0}
                          </div>
                          <div>
                            <strong>Failed:</strong> {execution.actionResults?.filter(r => !r.success).length || 0}
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  {executions.length === 0 && (
                    <Card>
                      <CardContent className="text-center py-12">
                        ...
                        <p className="text-muted-foreground">
                          Execute an automation to see execution history here
                        </p>
                      </CardContent>
                    </Card>
                  )}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Create support ticket when survey score is low
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-2 text-sm">
                        <div><strong>Trigger:</strong> Formbricks score &lt; 3</div>
                        <div><strong>Actions:</strong> Create Zammad ticket, Notify in chat</div>
                      </div>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Create project task from chat messages
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-2 text-sm">
                        <div><strong>Trigger:</strong> New chat message with #task</div>
                        <div><strong>Actions:</strong> Create Planka task</div>
                      </div>
                      <Button className="w-full mt-4" variant="outline">
                        Use Template
                      </Button>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Convert notes to wiki pages
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-2 text-sm">
                        <div><strong>Trigger:</strong> Note created with #wiki</div>
                        <div><strong>Actions:</strong> Create Outline page</div>
                      </div>
                      <Button className="w-full mt-4" variant="outline">
                        Use Template
                      </Button>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="space-y-2 text-sm">
                        <div><strong>Trigger:</strong> Daily at 9:00 AM</div>
                        <div><strong>Actions:</strong> Generate report, Send to chat</div>
                      </div>
                      <Button className="w-full mt-4" variant="outline">
                        Use Template
                      </Button>
